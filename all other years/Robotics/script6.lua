...
function sysCall_init()
    ...
    dummies_per_goal = N / 5
    for i = 1, 5 do
        x = goals[i][1]
        y = goals[i][2]
        for j = 1, dummies_per_goal do
            theta = math.rad(360 / dummies_per_goal * (j - 1))

            idx = (i - 1) * dummies_per_goal + j

            xArray[idx] = x
            yArray[idx] = y
            thetaArray[idx] = theta
            
            print(idx)

            weightArray[idx] = 1 / N
            dummyArray[idx] = sim.createDummy(0.05)
            sim.setObjectPosition(dummyArray[idx], -1, {x,y,0})
            sim.setObjectOrientation(dummyArray[idx], -1, {0,0,theta})
        end
    end

    setup_amount_turned = 0

    angle_to_rotate_setup = 5
    ...
    setupList[2] = {"stop"}
    setupList[3] = {"find_position"}
    setupList[4] = {"find_waypoint"}
...
function sysCall_actuation() 
    ...
    if (currentStep == "turn_by_angle") then
        ...
        turn_by(math.rad(angle_to_rotate_setup))
   ...
   -- MOVE
    ...
    if (isSetup) then
        ...
        elseif (currentStep == "stop") then
            ...
            for i = 1, N do
                thetaArray[i] = thetaArray[i] + angle_to_rotate_setup
